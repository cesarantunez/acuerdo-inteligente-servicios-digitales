...
(function () {
  ...
  const L = {
    projectType: (v) => (window.LABELS ? window.LABELS.projectType(LOCALE, v) : (v || "")),
    paymentMethod: (v) => (window.LABELS ? window.LABELS.paymentMethod(LOCALE, v) : (v || "")),
    currency: (v) => (window.LABELS ? window.LABELS.currency(LOCALE, v) : (v || "")),
  };
...
<p>Both parties acknowledge having sufficient legal capacity to enter into this Agreement and declare their free and voluntary consent.</p>`;
...

    {
      ...
      render(d) {
        const { esc } = H();
        const typeLabel = L.projectType(d.project.type);
        const heading = esc(d.project.name) || esc(typeLabel) || "Digital project";
        return `
<p>The <strong>Provider</strong> agrees to develop and deliver to the <strong>Client</strong> the following digital service:</p>
<div class="highlight">
  <p><strong>${heading}</strong>${typeLabel ? ` — ${esc(typeLabel)}` : ""}</p>
...
<p>Timelines may shift if the Client delays the delivery of required information, access credentials, or approvals.</p>`;
...

    {
      ...
      render(d) {
        ...
        const curLabel = L.currency(cur);
        const initial = (d.payment.total || 0) * ((d.payment.initialPercent || 0) / 100);
        const remaining = (d.payment.total || 0) - initial;
        const methodLabel = L.paymentMethod(d.payment.method);
        return `
<div class="highlight">
  <p><strong>Currency:</strong> ${esc(cur)} — ${esc(curLabel)}</p>
  ...
  <p><strong>Payment method:</strong> ${methodLabel ? esc(methodLabel) : "—"}</p>
