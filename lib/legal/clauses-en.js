...
<ul>
  ...
  <li>Maintain confidentiality regarding the Client's information.</li>
...

    {
      ...
      isHighlighted: true,
      render(d) {
        const lang = d?.metadata?.prevailingLanguage === "en" ? "ENGLISH" : "SPANISH";
        return `<p>This Agreement is presented in a bilingual version (Spanish and English) in side-by-side format. Both versions are official. In case of discrepancy, contradiction, or ambiguity between both versions, the Parties agree that the <strong>${lang}</strong> version shall prevail. The Parties declare having read and understood both versions before signing.</p>`;
      },
    },
